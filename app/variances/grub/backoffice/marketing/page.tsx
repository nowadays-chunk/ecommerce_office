...
export default function GrubMarketing() {
    const seeds = [
        { id: 'GR-MKT-101', code: 'ROOTS_ONLY', reward: '15% Soil Discount', status: 'Sprouting' },
        { id: 'GR-MKT-102', code: 'WILD_HARVEST', reward: 'Free_Seed_Batch', status: 'Full_Sun' },
        { id: 'GR-MKT-103', code: 'TRIBE_LINK_X', reward: 'Priority_Field_Entry', status: 'Resting' },
    ];
            ...
            <aside className="w-80 bg-white border-r-8 border-white p-12 flex flex-col justify-between shadow-2xl relative z-20">
                <div className="space-y-12">
                    ...
                    <nav className="space-y-8 text-[12px] font-black uppercase tracking-widest">
                        ...
                        <a href="#" className="block text-[#22c55e] underline decoration-8 underline-offset-8">COMMUNAL_GAMES</a>
            ...
            <main className="flex-1 p-24 space-y-32 h-screen overflow-y-auto relative bg-[#faf9f6]">
                <header className="flex justify-between items-center bg-white p-16 rounded-[5rem] shadow-2xl border-8 border-white group relative overflow-hidden">
                    ...
                    <div className="space-y-2 relative z-10">
                        <p className="text-[10px] font-black uppercase tracking-[0.5em] text-[#22c55e] italic">PROMOTION_OVERSIGHT // HARVEST_SYNC_LIVE</p>
                        <h1 className="text-7xl font-black tracking-tighter uppercase leading-none italic">THE_HARVEST_REWARDS.</h1>
                    </div>
                    <button className="px-16 py-8 bg-[#2d3a3a] text-white text-[10px] font-black uppercase tracking-widest rounded-full hover:bg-[#22c55e] transition-all shadow-2xl relative z-10">PLANT_NEW_REWARD</button>
                </header>
                ...
                <section className="bg-white p-20 rounded-[6rem] shadow-2xl border-8 border-white space-y-16 relative overflow-hidden">
                    <div className="flex justify-between items-center text-[12px] font-black uppercase tracking-widest text-[#2d3a3a]/40">
                        <div className="space-x-12">
                            <button className="text-[#22c55e] border-b-8 border-[#22c55e] pb-1">ALL_REWARDS</button>
                            <button className="hover:text-[#2d3a3a] transition-colors">SPROUTING</button>
                    ...
                    <table className="w-full text-left border-collapse border-separate border-spacing-y-8">
                        <thead>
                            <tr className="text-[12px] font-black uppercase tracking-widest opacity-40">
                                <th className="px-10 pb-6">REWARD_ID</th>
                                <th className="px-10 pb-6">ENERGY_CODE</th>
                                <th className="px-10 pb-6">ROOT_REWARD</th>
                                <th className="px-10 pb-6">HARVEST_STATUS</th>
                                <th className="px-10 pb-6 text-right">ACTIONS</th>
                            </tr>
                        </thead>
                        <tbody className="text-3xl font-black italic uppercase text-[#2d3a3a]">
                            {seeds.map((s) => (
                                <tr key={s.id} className="bg-[#faf9f6]/40 hover:bg-[#22c55e]/5 transition-colors group">
                                    <td className="p-10 rounded-l-[4rem]">#{s.id}</td>
                                    <td className="p-10 font-black tracking-widest uppercase text-[18px] text-[#2d3a3a]">{s.code}</td>
                                    <td className="p-10">{s.reward}</td>
                                    <td className="p-10 text-[#22c55e]">{s.status}</td>
